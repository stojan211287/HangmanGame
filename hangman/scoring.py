...
class HighScoreTable:
    def __init__(
        ...
        try:
            ...
            raise HighScoreTableConstructionError(
                "game_scoring_function must be a callable!"
            )
        ...
        self.high_score_file_path = os.path.join(
            self.high_score_persist_path, "high_scores.dat"
        )
        ...
        if os.path.exists(self.high_score_file_path):
            ...
            with open(self.high_score_file_path, "r") as high_score_file:
                ...
                for i, high_score_entry in enumerate(high_score_file.readlines()):
                    ...
                    deserialized_entry = self.serializer.deserialize_entry(
                        high_score_entry
                    )
    ...
    def score_and_store(self, player_name, game_word, no_of_mistakes_made):
        ...
        current_player_score = self.scoring_function(
            game_word=game_word, no_of_mistakes_made=no_of_mistakes_made
        )
    ...
    def _update_high_scores(self, new_scores):
        ...
        interim_high_scores = sorted(
            interim_high_scores, key=lambda entry: entry[1], reverse=True
        )
