...
from errors_backend import (
    GameAbortedAtStart,
    GuessNotAllowed,
    PlayerAlreadyGuessedCharacter,
    PlayerGuessWasWrong,
    PlayerLost,
    PlayerWon,
)
...
class HangmanGame:
    def __init__(self, game_rules, high_score_table, frontend):
        ...
        self.letters_already_guessed = {
            letter: False for letter in self.game_word_lookup.keys()
        }
    ...
    def _set_game_word_from_pool(self):
        ...
        for letter in game_word_string:
            game_word_lookup[letter] = self._get_letter_positions_in_word(
                letter, game_word_string
            )
    ...
    def _draw_game_state(self, game_message=None):
        ...
        print(os.linesep)
        print(
            "%s, you've made %d mistakes! You are allowed at most %d."
            % (
                self.player_name,
    ...
    def _check_guess(self, players_guess):
        ...
        try:
            if not self.letters_already_guessed[players_guess]:
                ...
                raise PlayerAlreadyGuessedCharacter(
                    "You already guessed %s!" % (players_guess,)
                )
    ...
    def _check_how_many_mistakes(self):
        ...
        if self.game_rules.too_many_mistakes(self.no_of_mistakes_made):
            raise PlayerLost(
                "You have made too many mistakes, %s! You lose." % (self.player_name,)
            )
        else:
            pass
    ...
    def _write_and_display_high_score(self):
        ...
        score_won = self.high_score_table.score_and_store(
            self.player_name, self.game_word, self.no_of_mistakes_made
        )
